...
FMT_CONSTEXPR auto join(const std::tuple<T...>& tuple,
    ...
    -> tuple_join_view<char16_t, T...> {
  return {tuple, sep};
}
template <typename... T>
FMT_CONSTEXPR auto join(const std::tuple<T...>& tuple,
                        basic_string_view<char32_t> sep)
    -> tuple_join_view<char32_t, T...> {
  return {tuple, sep};
}
template <typename T>
auto join(std::initializer_list<T> list, basic_string_view<char16_t> sep)
    -> join_view<const T*, const T*, char16_t> {
  return join(std::begin(list), std::end(list), sep);
}
template <typename T>
auto join(std::initializer_list<T> list, basic_string_view<char32_t> sep)
    -> join_view<const T*, const T*, char32_t> {
  return join(std::begin(list), std::end(list), sep);
}
